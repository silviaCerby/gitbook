...
Users with enough context on how to use DevTools could potentially leverage
them to access login credentials.
...
The Cerby CLI has the following updates:
  ...
  * We’ve enabled the `cerby-source` header in all CLI requests to prevent you from being blocked by our web application firewall (WAF) rules. The goal is to ensure a smoother and more reliable connection when you use the [CLI](https://help.cerby.com/en/articles/9118683-explore-the-cerby-cli).
