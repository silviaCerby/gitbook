...
**Super Admins** , **Admins** , and **Users** * Collection, business hub, and
asset **Owners** * Only supported using the Cerby web app {% endhint %}

As a collection **Owner** , you can remove an asset from a collection. When
you take this action, you must choose whether to remove or keep the access
that users and teams have to the asset through the collection.

To remove an asset from a collection, you must complete the following steps
...
{% hint style="info" %} **NOTE:** If in step 7 you selected to remove access
granted through the collection, Cerby initiates in the background automation
jobs to remove users from the external app only when users and teams don’t
have other access grants to the business hub. {% endhint %}
