# Disable vault backup in your workspace

**Description:** This article describes how to disable vault backup in your workspace.

{% hint style="info" %} **Who can use this capability?** * Workspace**Owners**
, **Super Admins** , and **Admins** * Only supported using the Cerby web app.
{% endhint %}

As a workspace **Owner** , **Super** **Admin** , or **Admin** , you can
disable the vault backup set up in your workspace.

**IMPORTANT:** After disabling the CyberArk-Cerby connection, accounts in
Cerby are no longer backed up in CyberArk; however, you can still access the
account previously backed up in CyberArk.  
---  
  
To disable vault backup in your workspace, you must complete the following
steps:

  1. Log in to your [Cerby](https://app.cerby.com/) workspace.

  2. Select the **Settings** option from the left navigation drawer. The **Workspace Configuration** page is displayed with the **General** tab active.

  3. Activate the **Privacy and security** tab.

  4. Activate the **Vault backup** left tab. 

  5. Click the **More options** () button. A drop-down menu is displayed.

  6. Select the **Disable connection** () option. The Disable connection message box is displayed.

  7. Click the **Confirm** button. The CyberArk-Cerby connection is displayed with the **Disabled** status.

Now you are done.

