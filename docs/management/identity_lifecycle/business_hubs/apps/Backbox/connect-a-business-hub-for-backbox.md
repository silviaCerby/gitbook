...
6. Enter and select your app information in the corresponding fields:
   ...
   * **User management and login method:** It is the way your users log in to the app and determines whether they must save their login credentials as a Cerby account connected to the business hub.
     1. Select the**Single sign-on (SSO)** option because access must be managed by your IdP, and users log in with SSO authentication. They are not asked to save their credentials in Cerby. **IMPORTANT:** The **Username and password** option is not currently available for Backbox.
