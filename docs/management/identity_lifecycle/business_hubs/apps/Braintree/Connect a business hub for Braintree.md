...
apps). {% endhint %}{% hint style="danger" %} **IMPORTANT:** Braintree
requires the **Merchant Accounts** field when creating user accounts.
Therefore, when adding users to your external app, you must specify the
merchant accounts to which the user should have access. {% endhint %}

This article provides instructions on how to connect a business hub for
Braintree. For other app-specific articles and videos, review the [Connecting
...
​**IMPORTANT:** The service account must indicate if this Braintree account is
for a sandbox or production environment. Therefore, when adding the account to
Cerby, ensure you enter **sandbox** in the **Team Name** field if the account
is connected to a sandbox environment, or leave the field empty for
production.
...
To connect a business hub for Braintree, you must complete the following main
...
To add a business hub and connect it to Braintree, you must complete the
following steps:
...
To connect your Braintree user account to the business hub so Cerby can manage
