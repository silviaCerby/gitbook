...
workspace belonging to one or multiple managed apps. As part of the setup, you
can specify the event that triggers the rotations.
...
To create a password policy for one or multiple apps, you must complete the
following steps:
...
**TIP:** You can use the search bar to look for specific apps, the drop-down
filter to show all apps or only apps with business hubs, or the **Select all**
option to select all apps in the list.
