...
members’ roles is not available for this business hub because Maze's basic
plan only supports one role and one **Owner.** All new users are assigned the
**Admin** role by default. {% endhint %}

This article provides instructions on how to connect a business hub for Maze.
...
To connect a business hub for Maze, you must complete the following main steps
...
To add a business hub and connect it to Maze, you must complete the following
steps:
...
To connect your Maze user account to the business hub so Cerby can manage and
