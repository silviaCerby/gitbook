...
{% hint style="danger" %} **IMPORTANT:** The Cerby browser extension is not
fully supported in incognito or private browsing windows**.** Some features
may not work as expected, which can lead to a bad user experience. For the
best performance and access to the full Cerby experience, use a standard
browser window. {% endhint %}
